...
.then(response => response.json())
    .then(data => {
      ...
      const tipo = data[24].tipo;
      const genero = data[24].genero.join(', ');
      const anio = data[24].anio;
      const pagina = data[24].pagina[0]
      const nombre = data[24].nombre
      ...
      posterImage.setAttribute('src', data[24].poster[0]);

      const descriptionElement = document.querySelector('#descripcion');
      descriptionElement.textContent = data[24].descripcion[1];
...
const data2 = [
  {
    "nombre":"Juego de Gemelas",
    "anio":"1998",
    "direccion":["Nancy Meyers"],
    "tipo":"Pelicula",
    "genero":["Aventura","Comedia","Drama"],
    "reparto":["Lindsay Lohan","Dennis Quaid","Natasha Richardson"],
    "ranking":"6,6",
    "duracion":"128"
  },
];
